...
namespace GSTEducationERP.Controllers
{
    public class CoordinatorController : Controller
    {
        ...
        private readonly BALCoordinator objbal;
 		BALBind objbind = new BALBind();
		    Coordinator objac = new Coordinator();
       ...
       [HttpGet]
		public async Task<ActionResult> UpdateLabAsyncVJ(string Labcode)
		{
			if (Session["StaffCode"] == null)
			{
				return RedirectToAction("Login", "Account");
       ...
       public async Task<ActionResult> UpdateLabAsyncVJ(Coordinator objC)
 {
     ...
     else
     {
        ...
        /// <returns> Lab Details.</returns>
        [HttpGet]
		public async Task<ActionResult> DetailsLabAsyncVJ(string Labcode)
		{

			Coordinator objdetials = new Coordinator();
			objdetials.BranchCode = Session["BranchCode"].ToString();
			objdetials.LabCode = Labcode;
...
        }
        ...
        public async Task<ActionResult> AddDemoKKAsync(string CourseName, string BatchName, string TrainerName, string SelectedStudentCodes, string NoOfStudent, string startTime, string endTime, string ExpectedDate, string LabCode, string StaffCode)
        {
            ...
            else
            {
			   ...
			   objDemo.BatchCode = await objbind.NewDemonCode(); 
...
if (DateTime.TryParse(ds.Tables[0].Rows[i]["StartTime"].ToString(), out startTime))
{
    d1.StartTime = startTime;
}
else
{
    d1.StartTime = DateTime.MinValue; 
}

if (DateTime.TryParse(ds.Tables[0].Rows[i]["EndTime"].ToString(), out endTime))
{
    d1.EndTime = endTime;
}
else
{
    d1.EndTime = DateTime.MinValue; 
}
...
if (DateTime.TryParse(ds.Tables[0].Rows[i]["StartTime"].ToString(), out startTime))
{
    d2.StartTime = startTime;
}
else
{
    d2.StartTime = DateTime.MinValue; 
}
    ...
    d2.EndTime = DateTime.MinValue; 
        ...
        public async Task<ActionResult> ListPendingEventPCAsync()
        {
            ...
            for (int i = 0; i < ds3.Tables[0].Rows.Count; i++)
            {
                ...
                DateTime startTime, endTime;
if (DateTime.TryParse(ds3.Tables[0].Rows[i]["StartTime"].ToString(), out startTime))
{
    d3.StartTime = startTime;
}
else
{
    d3.StartTime = DateTime.MinValue; 
}

if (DateTime.TryParse(ds3.Tables[0].Rows[i]["EndTime"].ToString(), out endTime))
{
    d3.EndTime = endTime;
}
else
{
    d3.EndTime = DateTime.MinValue; 
}
		...
		/// <summary>
		/// this method is written for grtting the update view created by the vishal pardeshi
		/// </summary>
		/// <param name="id"></param>
		/// <returns></returns>
		[HttpGet]
		public async Task<ActionResult> UpdateEventListPCAsync(int id)
		{
			if (Session["StaffCode"] == null)
			{
			    return RedirectToAction("Login", "Account");
			    ...
			    try
			    {
					...
					await BatchBindPCAsync();
					await StaffBindPCAsync();
					SqlDataReader dr = await objbal.GetEventDetailVPAsync(id);
					List<BreadcrumbItem> breadcrumbs = new List<BreadcrumbItem>
				{
					new BreadcrumbItem { Name = "Coordinator Dashboard", Url =Url.Action("CoordinatorDashboardPRAsync","Coordinator") },
					new BreadcrumbItem { Name = "Update Event" }
				};
			...
			if (dr.Read())
			{
				...
				objshow.EventOrgnaizerCode = dr["EventOrganizerName"].ToString();
		...
		/// <summary>
		/// this methode is wrritten for the update the evene and its status by Vishal Pardeshi
		/// </summary>
		/// <param name="obj"></param>
		/// <returns></returns>
		[HttpPost]
		 public async Task<JsonResult> UpdateEventListPCAsync(Coordinator obj)
        {
            ...
            {
                return Json(new { success = false, message = "Staff code is missing." }, JsonRequestBehavior.AllowGet);
            }
            await objbal.UpdateEventVPAsync(obj);
        ...
        public async Task<ActionResult> ListFeeFollowupAsyncRs()
        {
            ...
            {
                throw new Exception("BranchCode is not available in the session.");
            }
            await ListAllCourseAsyncSH();
            DataSet ds = await objbal.ListofInternalStudentFeesAsyncSH(BranchCode);
            List<Coordinator> lstData1 = new List<Coordinator>();
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                Coordinator obju = new Coordinator
                {
                    StudentCode = row["CandidateCode"]?.ToString() ?? string.Empty,
                    ...
                    ContactNumber = row["ContactNumber"] != DBNull.Value ? (row["ContactNumber"]).ToString() : "0",
                    CourseName = row["CourseName"]?.ToString() ?? string.Empty,
                    BatchName = row["BatchName"]?.ToString() ?? string.Empty,
                    CourseFee = row["CourseFee"] != DBNull.Value ? Convert.ToDouble(row["CourseFee"]).ToString("F2") : "0.00",
                    RegistrationFees = row["RegistrationFees"] != DBNull.Value ? Convert.ToDouble(row["RegistrationFees"]) : 0.0,
                    ...
                    RemainingFee = row["RemainingFees"] != DBNull.Value ? Convert.ToDouble(row["RemainingFees"]).ToString("F2") : "0.00",
                    InstallmentAmount = row["NextInstallment"] != DBNull.Value ? Convert.ToDouble(row["NextInstallment"]).ToString("F2") : "0.00",
                    //  LastInstallmentDate = row["LastInstallmentDate"] != DBNull.Value ? Convert.ToDateTime(row["LastInstallmentDate"]).ToString("yyyy-MM-dd") : DateTime.MinValue.ToString("yyyy-MM-dd"),
                    ...
                    Duration = row["NextInstallmentDate"] != DBNull.Value ? Convert.ToDateTime(row["NextInstallmentDate"]).ToString("dd/MM/yyyy") : string.Empty,
                    BranchAddress = row["BranchAddress"]?.ToString() ?? string.Empty,

                };

                DataSet dsStudFee = await objbal.GetFeesDataForSingleStudent(BranchCode, obju.StudentCode);
                ...
                foreach (DataRow row1 in dsStudFee.Tables[0].Rows)
                {
                    try
                    {
                        ...
                        double givenNoOfInstallment = Convert.ToDouble(row1["NoofInstallment"]);
                        double totalFees = Convert.ToDouble(row1["TotalFees"]);
                        double totalPaid = Convert.ToDouble(row1["TotalPaid"]);
                        objac.Duration = row1["InstallmentDuration"].ToString();

                        // Calculate unpaid installments
                        List<Coordinator> upcomingInstallments = GetUnpaidInstallments(totalFees, totalPaid, givenNoOfInstallment);
                        ...
                        foreach (var installment in upcomingInstallments)
                        {
                            if (remainingTotalPaid <= installment.TotalCompletedAmount)
                            {
                                ...
                                obju.Duration = installment.InstallmentDate.ToString("dd-MM-yyyy");
                                obju.InstallmentAmount = Math.Round(nextInstallmentAmount, 2).ToString();
                        ...
                        List<Coordinator> InstallmentNo = CalculateInstallments(totalFees, givenNoOfInstallment);
            ...
            Coordinator obj = new Coordinator { lstfeefollowup = lstData1 };
            ...
            List<BreadcrumbItem> breadcrumbs = new List<BreadcrumbItem>
         {
            ...
            ViewBag.Breadcrumbs = breadcrumbs;
            return PartialView("ListFeeFollowupAsyncRs", obj);
        ...
        [HttpGet]
        public async Task<ActionResult> ViewFeeFollowupDetailsAsyncRs(string StudentId)
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
            ...
            else
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    objC1.CourseFee = Convert.ToDecimal(ds.Tables[0].Rows[i]["CourseFee"]).ToString("#,0") + " " + "Rs.";
                    objC1.Discount = Convert.ToInt32(ds.Tables[0].Rows[i]["Discount"].ToString());
                    objC1.RemainingFee = Convert.ToDecimal(ds.Tables[0].Rows[i]["RemainingFees"]).ToString("#,0") + " " + "Rs.";
                    objC1.PaidAmount = Convert.ToDecimal(ds.Tables[0].Rows[i]["TotalPaid"]).ToString("#,0") + " " + "Rs.";
                    objC1.DiscountedCourseFee = Convert.ToDecimal(ds.Tables[0].Rows[i]["TotalFees"]).ToString("#,0") + " " + "Rs.";
                    objC1.InstallmentAmount = Math.Round(Convert.ToDecimal(ds.Tables[0].Rows[i]["NextInstallment"])).ToString("#,0") + " " + "Rs.";

                    string nextInstallmentDate = ds.Tables[0].Rows[i]["NextInstallmentDate"].ToString();
                    DateTime Date;
                    if (DateTime.TryParse(nextInstallmentDate, out Date))
                    {
                    ...
                    objC1.nextdate = Date.ToString("dd-MM-yyyy");
                    string lastInstallmentDate = ds.Tables[0].Rows[i]["LastInstallmentDate"]?.ToString();
                    DateTime LastDate;

                    if (DateTime.TryParse(lastInstallmentDate, out LastDate))
                    {
                        objC1.LastInstallmentDate = LastDate.ToString("dd-MM-yyyy");
                    }
        ...
        [HttpGet]
        public async Task<ActionResult> AddFeeFollowUpDetailsAsyncRS(/*string StudentCode*/
 string StudentCode,
 string FullName,
 ...
 string NextInstallmentDate)
        {
            ...
            else
            {
                ...
                objC.StudentCode = StudentCode;
                objC.FullName = FullName;
                objC.ContactNumber = ContactNumber;
                objC.Emailid = EmailId.ToString();
                objC.CourseName = CourseName;
                objC.CourseFee = CourseFee.ToString();
                objC.RemainingFee = RemainingFee.ToString();
                objC.InstallmentAmount = InstallmentAmount.ToString();
                objC.StaffCode = Session["StaffName"].ToString();
                // objC.NextInstallmentDate = NextInstallmentDate;



                DataSet ds = new DataSet();
                ds = await objbal.GetCandidateEnqFollowUpHistory(objC);
                List<Coordinator> ListEnquiry = new List<Coordinator>();
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    Coordinator objC1 = new Coordinator();
                ...
                return PartialView("AddFeeFollowUpDetailsAsyncRS", objC);
            }
        }
        ...
        public async Task<ActionResult> AddFeeFollowUpDetailsAsyncRS(Coordinator ObjCo)
        {
            ...
            try
            {
                ...
                return Json(new { success = true, message = "Data saved successfully", redirectUrl = Url.Action("studentFeeMainpageasyncSH", "Coordinator") });
            }
            catch (Exception ex)
            {
                // Log the error (use a logging framework)
                return Json(new { success = false, message = "An error occurred while saving the data. Please try again." });
            }
        }
 ...
 public List<Coordinator> CalculateInstallments(double TotalFees, double GivenNoOfInstallment)
 {
     List<Coordinator> installments = new List<Coordinator>();
     ...
     DateTime currentInstallmentDate = objac.BatchStartDate.AddDays( Convert.ToInt32(objac.Duration));
     ...
     for (int i = 0; i < GivenNoOfInstallment; i++)
     {
         ...
         installments.Add(new Coordinator
         {
         ...
         currentInstallmentDate = currentInstallmentDate.AddDays(Convert.ToInt32(objac.Duration));
     }
     return installments;
 }


        public List<Coordinator> GetUnpaidInstallments(double totalFees, double totalPaid, double givenNoOfInstallment)
        {
            List<Coordinator> allInstallments = CalculateInstallments(totalFees, givenNoOfInstallment);
            List<Coordinator> unpaidInstallments = new List<Coordinator>();
            ...
            foreach (var installment in allInstallments)
            {
                if (remainingTotalPaid >= installment.TotalCompletedAmount)
                {
                    ...
                    {
                        unpaidInstallments.Add(new Coordinator
                        {
                            InstallmentDate = installment.InstallmentDate,
                            InstallmentAmount = remainingTotalPaid.ToString(),
                ...
                else
                {
                    ...
                    if (remainingAmount > 0)
                    {
                        ...
                        {
                            InstallmentDate = installment.InstallmentDate,
                            InstallmentAmount = remainingAmount.ToString(),
                            TotalCompletedAmount = installment.TotalCompletedAmount,
                            InstallmentNo = installment.InstallmentNo
                        });
                    }
        ...
        public async Task<ActionResult> ListInternalStudentasyncSH()
        {
            ...
            {
                List<Coordinator> Mo = await InternalStudentFeeDataAsyncSH();
                Coordinator obj = new Coordinator();
                obj.lstStudentFees = Mo;
                List<BreadcrumbItem> breadcrumbs = new List<BreadcrumbItem>
          {
            //new BreadcrumbItem { Name = "Home", Url = "/" },
            new BreadcrumbItem { Name = "Coordinator Dashboard", Url = "AccountantDashboardAsyncSGS/Coordinator" },
            new BreadcrumbItem { Name = "Fee Collection", Url = "studentFeeMainpageasyncSH/Coordinator" },
            new BreadcrumbItem { Name = "Internal Student", Url = "ListInternalStudentasyncSH/Coordinator" }
          };
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<Coordinator>> InternalStudentFeeDataAsyncSH()
        {
            ...
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                Coordinator obju = new Coordinator
                {
                    ...
                    CourseFeeAD = row["CourseFee"] != DBNull.Value ? Convert.ToDouble(row["CourseFee"]) : 0.0,
                ...
                foreach (DataRow row1 in dsStudFee.Tables[0].Rows)
                {
                    try
                    {
                        ...
                        double totalFees = Convert.ToDouble(row1["TotalFeesWRpaid"]);
                        double totalPaid = Convert.ToDouble(row1["TotalPaid"]);



                        //objac.Duration = Convert.ToInt32(row1["InstallmentDuration"]).ToString();
                        string installmentDuration = row1["InstallmentDuration"]?.ToString(); // Get the value as a string
                        if (!string.IsNullOrWhiteSpace(installmentDuration))
                        {
                            string numericPart = new string(installmentDuration.Where(char.IsDigit).ToArray());
                            if (int.TryParse(numericPart, out int duration))
                            {
                                objac.Duration = duration.ToString(); // Assign the numeric part
                            }
                            else
                            {
                                objac.Duration = "0"; // Default value if no numeric part is found
                            }
                        }
                        else
                        {
                            objac.Duration = "0"; // Default value for null or empty input
                        }
        ...
        public async Task<ActionResult> FilterFeeCollection(string TabID, string Course, string Batch, DateTime? startDate, DateTime? endDate, string dateFilterType)
        {
            ...
            else
            {
                try
                {
                    if (TabID == "internal")
                    {
                        List<Coordinator> StudentIntFees = Session["ListforIntFilter"] as List<Coordinator>;
                        ...
                        Coordinator obj = new Coordinator { lstStudentFees = StudentIntFees };
                        return await Task.Run(() => PartialView("_ListInternalStudentasyncSH", obj));
                    }
                    else if (TabID == "external")
                    {
                        List<Coordinator> StudentExtFees = Session["ListforExtFilter"] as List<Coordinator>;
                        ...
                        Coordinator obj = new Coordinator { lstStudentFees = StudentExtFees };
                        return await Task.Run(() => PartialView("_ListExternalStudentasyncSH", obj));
                    }
                    else if (TabID == "Pending")
                    {
                        List<Coordinator> StudentPendingFees = Session["PendingFilterList"] as List<Coordinator>;
                        ...
                        // Prepare the model for view
                        Coordinator obj = new Coordinator { LstPendindFeeStud = StudentPendingFees };
