    ...
    public class BindController : Controller
    {
        ...
        public async Task<ActionResult> DetailStaffProfile()
        {
            ...
            else
            {
             ...
             {
             new BreadcrumbItem { Name = "Counsellor Dashborad", Url = Url.Action("MainDashboardAsyncHP","Counsellor")},
        ...
        [HttpPost]
        public async Task<ActionResult> DetailStaffProfile(string param1, string param3, HttpPostedFileBase UploadDocument)
        {
            Bind ObjCo = new Bind();
            ObjCo.StaffCode = Session["StaffCode"].ToString();

            ObjCo.UploadDocument = UploadDocument;
            if (ObjCo.UploadDocument != null)
            {
                //var name = ObjCo.StaffName;
                newFileName = Path.GetFileNameWithoutExtension(ObjCo.UploadDocument.FileName);
                extention = Path.GetExtension(ObjCo.UploadDocument.FileName);
                newFileName = newFileName + extention;
                file = "~/Content/Admin/docs" + newFileName;
            }
            ...
            if (param3 == "StaffName")
            {
                ObjCo.StaffName = param1;
                ObjCo.Document = file;
                newFileName = Path.Combine(Server.MapPath("~/Content/Admin/docs"), newFileName);
                ObjCo.UploadDocument.SaveAs(newFileName);
                await BALObj.StaffFEditRequestAsync(ObjCo);
            }
            if (param3 == "StaffDOB")
            {
                ObjCo.DOB = DateTime.Parse(param1);
                ObjCo.Document = file;
                newFileName = Path.Combine(Server.MapPath("~/Content/Admin/docs"), newFileName);
                ObjCo.UploadDocument.SaveAs(newFileName);
                await BALObj.StaffFEditRequestAsync(ObjCo);
            }
            if (param3 == "StaffPhoneNo")
            {
                ObjCo.PhoneNo = param1;
                if (ObjCo.UploadDocument != null)
                {
                    ObjCo.Document = file;
            ...
            if (param3 == "StaffEmailId")
            {
                ObjCo.EmailId = param1;
                ObjCo.Document = file;

                await BALObj.StaffFEditRequestAsync(ObjCo);
            }
            if (param3 == "CurrentAddress")
            {
                ObjCo.CurrentAddress = param1;
                ObjCo.Document = file;
                newFileName = Path.Combine(Server.MapPath("~/Content/Admin/docs"), newFileName);
                ObjCo.UploadDocument.SaveAs(newFileName);
                await BALObj.StaffFEditRequestAsync(ObjCo);
            }
            if (param3 == "StaffMaritalStatus")
            {
                ObjCo.MaritalStatus = param1;
                ObjCo.Document = file;
                newFileName = Path.Combine(Server.MapPath("~/Content/Admin/docs"), newFileName);
                ObjCo.UploadDocument.SaveAs(newFileName);
                await BALObj.StaffFEditRequestAsync(ObjCo);
            }
            if (param3 == "StaffEmergencyContactNo")
            {
                ObjCo.EmergencyContactNo = param1;
                ObjCo.Document = file;

                await BALObj.StaffFEditRequestAsync(ObjCo);
            }

            if (param3 == "StaffEmergencyContactAddress")
            {
                ObjCo.EmergencyContactAddress = param1;
                ObjCo.Document = file;
                newFileName = Path.Combine(Server.MapPath("~/Content/Admin/docs"), newFileName);
                ObjCo.UploadDocument.SaveAs(newFileName);
                await BALObj.StaffFEditRequestAsync(ObjCo);
            }
            if (param3 == "StaffFatherContactNo")
            {
                ObjCo.FatherContactNo = param1;
                ObjCo.Document = file;

                await BALObj.StaffFEditRequestAsync(ObjCo);
            }
            if (param3 == "StaffMotherContactNo")
            {
                ObjCo.MotherContactNo = param1;
                ObjCo.Document = file;
        ...
        public async Task<ActionResult> DetailsStaffProfileAsync()
        {
...

            {
                ...
                while (dr.Read())
                {
                    ...
                    ObjCo.DOB = Convert.ToDateTime(dr["DOB"].ToString());
                    ObjCo.PhoneNo = dr["PhoneNo"].ToString();
                    ObjCo.EmailId = dr["EMail"].ToString();
 ...
 ObjCo.BranchCode = Session["BranchCode"].ToString();
         string Staffpostion = Session["StaffPositionId"].ToString();
                ...
                //{
                  if (Staffpostion == "3")
                {
                    ds = await BALObj.GetDataBatchAsyncST(ObjCo);
                }// Pass CourseCode to GetDataBatchAsyncST
                else 
                {
                    ds = await BALObj.GetAllBatchesAM(ObjCo);
                } // Pass CourseCode to GetDataBatchAsyncST
                                                              //}
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
				{
					...
					coObj.BatchCode = ds.Tables[0].Rows[i]["BatchCode"] != DBNull.Value
						? ds.Tables[0].Rows[i]["BatchCode"].ToString()
						: string.Empty;

					coObj.BatchName = ds.Tables[0].Rows[i]["BatchName"] != DBNull.Value
						? ds.Tables[0].Rows[i]["BatchName"].ToString()
						: string.Empty;

					coObj.CourseName = ds.Tables[0].Rows[i]["CourseName"] != DBNull.Value
						? ds.Tables[0].Rows[i]["CourseName"].ToString()
						: string.Empty;

					coObj.NoOfStudent = int.TryParse(ds.Tables[0].Rows[i]["NoOfStudent"].ToString(), out int noOfStudents)
						? noOfStudents
						: 0;

					coObj.CreateDate = DateTime.TryParse(ds.Tables[0].Rows[i]["CreateDate"].ToString(), out DateTime createDate)
						? createDate
						: DateTime.MinValue;

					coObj.Status = ds.Tables[0].Rows[i]["Status"] != DBNull.Value
						? ds.Tables[0].Rows[i]["Status"].ToString()
						: string.Empty;

					coObj.StaffName = ds.Tables[0].Rows[i]["StaffName"] != DBNull.Value
						? ds.Tables[0].Rows[i]["StaffName"].ToString()
						: string.Empty;
       ...
       [HttpPost]
        public async Task<ActionResult> RegisterBatchAsyncST(string BatchName, string CourseCode, string StudentCode, string NoOfStudent, DateTime TentativeBatchDate, int BatchCapacity)
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
            ...
            else
            {
                ...
                ObjCo.TentativeBatchDate = TentativeBatchDate;
                ObjCo.BatchCapacity = BatchCapacity;
                ObjCo.StatusId = 44;
                ObjCo.TypeId = 5;
        ...
        public async Task<JsonResult> GetSelectedBatchAsyncST()
        {
            ...
            Bind objb = new Bind();
			 ...
			 ds = await BALObj.GetSelectedBatchAsyncST(objb);
        ...
        private async Task<List<Bind>> ListStudShAsync(int Statusid, string CourseCode)
        {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    Bind objS = new Bind();
					objS.Id = int.Parse(ds.Tables[0].Rows[i]["SerialNumber"].ToString());
                    ...
                    DateTime date;
					 if (DateTime.TryParse(ds.Tables[0].Rows[i]["AdmmissionDate"].ToString(), out date))
					 {
        ...
        public async Task<ActionResult> ListReleaseStudentSHAsync(string StudentCode)
        {
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
				obj.Id = int.Parse(ds.Tables[0].Rows[i]["SerialNumber"].ToString());
                obj.StudentCode = ds.Tables[0].Rows[i]["CandidateCode"].ToString();
                obj.CourseName = ds.Tables[0].Rows[i]["CourseName"].ToString();
                obj.BatchName = ds.Tables[0].Rows[i]["BatchName"].ToString();
                obj.StudentName = ds.Tables[0].Rows[i]["FullName"].ToString();
        ...
        [HttpGet]
       public async Task<ActionResult> ListTaskManagementAsyncPG(DateTime? fromDate = null, DateTime? toDate = null, int  status =0)
{
    if (Session["StaffCode"] == null)
    {
        return RedirectToAction("Login", "Account");
    ...
    else
    {
        try
        {
            ...
            DataSet ds = await BALObj.ListTaskManagementAsyncPG(objT, fromDate, toDate ,status);
                ...
                List<BreadcrumbItem> breadcrumbs = new List<BreadcrumbItem>
                 {
                      ...
                      new BreadcrumbItem { Name = "Task Management" }
                 };
                ViewBag.Breadcrumb = breadcrumbs;
            }
            if (Staffpostion == "2")
        ...
        //----------------------------------------------Adiraj--------------------------------------//
        /// <summary>
        /// This is Empty View Used for Binding Staffwise LeaveCount & StaffAppliedLeaves List
        /// </summary>
        /// <returns></returns>
        public async Task<ActionResult> LeaveManagementAsyncAM()
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
        ...
        /// <summary>
        /// This Method is used for Binding LeaveCounts
        /// </summary>
        /// <returns>Staff wise Leave Counts</returns>
        [HttpGet]
        public async Task<ActionResult> ListLeaveCountsAsyncAM()
        {
            try
            {
                await LeaveTypesAndCount();
                return PartialView("ListLeaveCountsAsyncAM");
            }
            catch (Exception ex)
            {
                throw (ex);
        ...
        /// <summary>
        /// Getting leave types and count 
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        public async Task<ActionResult> LeaveTypesAndCount()
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
            ...
            else
            {
                try
                {
                    List<string> LeaveTypesColumn = new List<string>();
                    List<string> LeaveCountsColumn = new List<string>();
                    List<string> LeaveTypeIdColumn = new List<string>();
                    Bind objLeave = new Bind();
                    objLeave.StaffCode = Session["StaffCode"].ToString();
                    objLeave.StaffPositionid = int.Parse(Session["StaffPositionId"].ToString());
                    DataSet ds = new DataSet();
                    ds = await BALObj.GetLeaveCountAsyncAM(objLeave);
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        Bind objLeaveCount = new Bind();
                        objLeaveCount.LeaveType = ds.Tables[0].Rows[i]["LeaveType"].ToString();
                        objLeaveCount.LeaveCount = Convert.ToDouble(ds.Tables[0].Rows[i]["RemainingLeaveCount"].ToString());
                        objLeaveCount.LeaveTypeId = Convert.ToInt32(ds.Tables[0].Rows[i]["LeaveId"].ToString());
                        LeaveTypesColumn.Add(objLeaveCount.LeaveType);
                        LeaveCountsColumn.Add(objLeaveCount.LeaveCount.ToString());
                        LeaveTypeIdColumn.Add(objLeaveCount.LeaveTypeId.ToString());
                    }
                    ViewBag.LeaveTypes = LeaveTypesColumn;
                    ViewBag.LeaveCounts = LeaveCountsColumn;
                    ViewBag.LeaveTypeId = LeaveTypeIdColumn;

                    List<string> AllotteLeavesList = new List<string>();
                    DataSet dsAllottedLeaves = new DataSet();
                    dsAllottedLeaves = await BALObj.GetAllottedLeavesAsyncAM(objLeave);
                    for (int i = 0; i < dsAllottedLeaves.Tables[0].Rows.Count; i++)
                    {
                        Bind objLeaveCount = new Bind();
                        objLeaveCount.AllottedLeaveCount = Convert.ToInt32(dsAllottedLeaves.Tables[0].Rows[i]["LeaveCount"].ToString());
                        AllotteLeavesList.Add(objLeaveCount.AllottedLeaveCount.ToString());
                    }
                    ViewBag.AllottedLeavesList = AllotteLeavesList;
                    return View();
                }
                catch (Exception ex)
                {
        ...
        /// <summary>
        /// Getting List of all types of leaves of staff ( pending, approved and Rejected )
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        public async Task<ActionResult> ListStaffAppliedLeavesAsyncAM()
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
            ...
            else
            {
                try
                {
                    ...
                    objLeave.StaffCode = Session["StaffCode"].ToString();
                    DataSet ds = new DataSet();
                    ds = await BALObj.GetAppliedLeavesListAsyncAM(objLeave);
                    List<Bind> ListStaffAppliedLeaves = new List<Bind>();
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        Bind objLeaveCount = new Bind();
                        objLeaveCount.LeaveAppliedId = Convert.ToInt32(ds.Tables[0].Rows[i]["ApplyLeaveId"].ToString());
                        objLeaveCount.LeaveType = ds.Tables[0].Rows[i]["LeaveType"].ToString();
                        objLeaveCount.Day = ds.Tables[0].Rows[i]["Day"].ToString();
                        objLeaveCount.StartDate = Convert.ToDateTime(ds.Tables[0].Rows[i]["StartDate"].ToString());
                        objLeaveCount.EndDate = Convert.ToDateTime(ds.Tables[0].Rows[i]["EndDate"].ToString());
                        objLeaveCount.NoOfDays = float.Parse(ds.Tables[0].Rows[i]["NoOfDays"].ToString());
                        objLeaveCount.LeaveLocation = ds.Tables[0].Rows[i]["Location"].ToString();
                        objLeaveCount.Reason = ds.Tables[0].Rows[i]["Reason"].ToString();
                        objLeaveCount.Document = ds.Tables[0].Rows[i]["UploadDocuments"].ToString();
                        objLeaveCount.Status = ds.Tables[0].Rows[i]["Status"].ToString();
                        objLeaveCount.StatusId = Convert.ToInt32(ds.Tables[0].Rows[i]["StatusId"].ToString());
                        ListStaffAppliedLeaves.Add(objLeaveCount);
                    }
                    objLeave.ListStaffAppliedLeaves = ListStaffAppliedLeaves;
                    return PartialView("ListStaffAppliedLeavesAsyncAM", objLeave);
                }
                catch (Exception ex)
                {
                    throw (ex);
        ...
        /// <summary>
        /// Getting Types of Leaves available for particular logged in Staff.
        /// </summary>
        /// <returns></returns>
        public async Task<JsonResult> GetLeaveTypes()
        {
            try
            {
                Bind ObjCo = new Bind();
                DataSet ds = new DataSet();
                ObjCo.StaffCode = Session["StaffCode"].ToString();
                ds = await BALObj.GetLeaveTypes(ObjCo);
                List<SelectListItem> LeaveTypesList = new List<SelectListItem>();
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    LeaveTypesList.Add(new SelectListItem
                    {
                        Text = dr["LeaveType"].ToString(),
                        Value = dr["LeaveId"].ToString()
                    });
                }
                ViewBag.LeaveType = LeaveTypesList;
                return Json(LeaveTypesList, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                ...
                return Json(new { success = false, message = "An error occurred while fetching leave types." }, JsonRequestBehavior.AllowGet);
            }
        }

        /// <summary>
        /// This method is used Passing the data to the Database GSTtblLeave.
        /// </summary>
        /// <param name="obj"> This obj gets the values from the View and passes the required values to database through function which is declared in BALBind.</param>
        /// <returns></returns>
        [HttpPost]
        public async Task<ActionResult> ListStaffAppliedLeavesAsyncAM(Bind obj)
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
            ...
            else
            {
                try
                {
                    ...
                    if (obj.UploadDocument != null)
                    {
                        var newFileName = Path.GetFileNameWithoutExtension(obj.UploadDocument.FileName);
                        var extension = Path.GetExtension(obj.UploadDocument.FileName);
                        newFileName = newFileName + extension;
                        var file = "~/Content/Admin/docs/LeaveDocs/" + newFileName;
                        obj.Document = file;
                        newFileName = Path.Combine(Server.MapPath("~/Content/Admin/docs/LeaveDocs/"), newFileName);
                        obj.UploadDocument.SaveAs(newFileName);
                    }
                    if (obj.Day == "Short Leave")
                    {
                        obj.LeaveAppliedId = 10;
                    }
                    await BALObj.SaveAppliedLeaveAsyncAM(obj);
                    await ListStaffAppliedLeavesAsyncAM();
                    return Json(new { success = true, message = "Leave Applied successfully" });
                }
                catch (Exception ex)
                {
                   
                    return Json(new { success = false, message = "An error occurred while applying for leave." });
                }
            }
        }

        /// <summary>
        /// This method is used to get the selected applied leave ID data from database and display it in the EditLeave View.
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        public async Task<ActionResult> EditLeaveAsyncAM(int id)
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
            ...
            else
            {
                try
                {
                    await LeaveTypesAndCount();
                    Bind objB = new Bind();
                    objB.LeaveAppliedId = id;
                    SqlDataReader dr;
                    dr = await BALObj.GetAppliedLeaveAsyncAM(objB);
                    while (dr.Read())
                    {
                        objB.LeaveAppliedId = Convert.ToInt32(dr["ApplyLeaveId"].ToString());
                        objB.LeaveTypeId = Convert.ToInt32(dr["LeaveTypeId"].ToString());
                        objB.LeaveType = dr["LeaveType"].ToString();
                        objB.Day = dr["Day"].ToString();
                        objB.StartDate = Convert.ToDateTime(dr["StartDate"].ToString());
                        objB.EndDate = Convert.ToDateTime(dr["EndDate"].ToString());
                        objB.NoOfDays = Convert.ToInt32(dr["NoOfDays"].ToString());
                        objB.LeaveLocation = dr["Location"].ToString();
                        objB.Reason = dr["Reason"].ToString();
                        objB.Document = dr["UploadDocuments"].ToString();
                        objB.StatusId = Convert.ToInt32(dr["StatusId"].ToString());
                        objB.Status = dr["Status"].ToString();
                    }
                    return PartialView("EditLeaveAsyncAM", objB);
                }
                catch (Exception ex)
                {
                    throw (ex);
        ...
        /// <summary>
        /// This method is used to get the Company's annual holidays from database and display it in Calender.
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        public async Task<ActionResult> ListOrganisationHolidaysAsyncAM()
        {
            if (Session["StaffCode"] == null)
            {
                return RedirectToAction("Login", "Account");
            ...
            else
            {
                ...
                {
                    Bind objOH = new Bind();
                    objOH.StaffCode = Session["StaffCode"].ToString();
                    objOH.BranchCode = Session["BranchCode"].ToString();
                    DataSet ds = new DataSet();
                    ds = await BALObj.GetAnnualHolidaysOfCompanyAsyncAM(objOH);
                    List<Bind> LstOrgHolidays = new List<Bind>();
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        Bind objLeaveCount = new Bind();
                        objLeaveCount.LeaveType = ds.Tables[0].Rows[i]["HolidayName"].ToString();
                        objLeaveCount.StartDate = Convert.ToDateTime(ds.Tables[0].Rows[i]["HolidayDate"].ToString());
                        LstOrgHolidays.Add(objLeaveCount);
                    }
                    ViewBag.OrgHolidays = LstOrgHolidays;
                    return PartialView("ViewCalenderPCAsync", "Coordinator");
                }
                catch (Exception ex)
                {
                    throw (ex);
        ...
        /// <summary>
        /// This method is used for Validating the short leave time.
        /// </summary>
        /// <returns></returns>
        public async Task<JsonResult> ValidateShortLeaveTimeAsyncAM()
        {
            try
            {
                Bind objOH = new Bind();
                ...
                objOH.BranchCode = Session["BranchCode"].ToString();
                DataSet ds = new DataSet();
                ds = await BALObj.ValidatingShortLeveTime(objOH);

                List<Bind> GetShortLeaveTime = new List<Bind>();
                if (ds.Tables[0].Rows.Count > 0)
                {
                    DataRow row = ds.Tables[0].Rows[0];
                    string CountofShortLeave = row["LeaveCount"].ToString();
                    ViewBag.ShortLeaveMaxTime = new { CountofShortLeave };
                }
                var ShortLeaveTime = ViewBag.ShortLeaveMaxTime;
                return Json(ShortLeaveTime, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                // Log the exception
                // You can use any logging framework like log4net, NLog, Serilog, etc.
                // Example: Logger.Error(ex);

                // Return an error message
                return Json(new { success = false, message = "An error occurred while validating short leave time." }, JsonRequestBehavior.AllowGet);
            }
        }

        /// <summary>
        /// This method is used for validating CompOff Date which is selected by the user.
        /// </summary>
        /// <param name="date"></param>
        /// <returns></returns>
        [HttpPost]
        public async Task<ActionResult> ValidateCompOffAsyncAM(string date)
        {
            try
            {
                if (Session["StaffCode"] == null)
                ...
                else
                {
                    string Date;
                    Bind objB = new Bind();
                    objB.StaffCode = Session["StaffCode"].ToString();
                    objB.HolidayWorkedDate = Convert.ToDateTime(date);
                    SqlDataReader dr;
                    dr = await BALObj.ValidateCompOffDate(objB);
                    while (dr.Read())
                    {
                        objB.Date = Convert.ToDateTime(dr["Date"].ToString());
                        objB.Message = dr["Message"].ToString();
                    }

                    Date = objB.Date.ToString("yyyy-MM-dd HH:mm:ss.fff");
                    DateTime Datee = Convert.ToDateTime(Date);
                    Date = Datee.ToString("yyyy-MM-dd");

                    string DisplayMessage = objB.Message;
            ...
            catch (Exception ex)
            {
                ...
                // Return an error message
                return Json(new { success = false, message = "An error occurred while validating Comp Off date." }, JsonRequestBehavior.AllowGet);
            }
        }

        /// <summary>
        /// This method is used for getting leave requests according to logged in person's Reporting Staff.
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        public async Task<ActionResult> GetLeaveRequestsAsyncAM()
        {
            try
            {
                if (Session["StaffCode"] == null)
                ...
                {
                    Bind objLeaveR = new Bind();
                    objLeaveR.StaffCode = Session["StaffCode"].ToString();
                    DataSet ds = new DataSet();
                    ds = await BALObj.GetLeaveRequestsAM(objLeaveR);
                    List<Bind> ListStaffAppliedLeaves = new List<Bind>();
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        Bind objLeaveRequests = new Bind();
                        objLeaveRequests.LeaveAppliedId = Convert.ToInt32(ds.Tables[0].Rows[i]["ApplyLeaveId"].ToString());
                        objLeaveRequests.LeaveType = ds.Tables[0].Rows[i]["LeaveType"].ToString();
                        objLeaveRequests.StaffName = ds.Tables[0].Rows[i]["StaffName"].ToString();
                        objLeaveRequests.Day = ds.Tables[0].Rows[i]["Day"].ToString();
                        objLeaveRequests.StartDate = Convert.ToDateTime(ds.Tables[0].Rows[i]["StartDate"].ToString());
                        objLeaveRequests.EndDate = Convert.ToDateTime(ds.Tables[0].Rows[i]["EndDate"].ToString());
                        objLeaveRequests.NoOfDays = float.Parse(ds.Tables[0].Rows[i]["NoOfDays"].ToString());
                        objLeaveRequests.LeaveLocation = ds.Tables[0].Rows[i]["Location"].ToString();
                        objLeaveRequests.Reason = ds.Tables[0].Rows[i]["Reason"].ToString();
                        objLeaveRequests.Document = ds.Tables[0].Rows[i]["UploadDocuments"].ToString();
                        objLeaveRequests.Status = ds.Tables[0].Rows[i]["Status"].ToString();
                        objLeaveRequests.StatusId = Convert.ToInt32(ds.Tables[0].Rows[i]["StatusId"].ToString());
                        ListStaffAppliedLeaves.Add(objLeaveRequests);
                    }
                    objLeaveR.ListLeaveRequests = ListStaffAppliedLeaves;
                    int CountOfRequests = ListStaffAppliedLeaves.Count;
                    ViewBag.LeaveRequestsCount = CountOfRequests;
                    return PartialView(objLeaveR);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<ActionResult> UpdateLeaveRequestAsyncAM(int id)
        {
            try
            {
                if (Session["StaffCode"] == null)
                ...
                else
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        objB.StartDate = Convert.ToDateTime(dr["StartDate"].ToString());
                        objB.EndDate = Convert.ToDateTime(dr["EndDate"].ToString());
                        objB.NoOfDays = float.Parse(dr["NoOfDays"].ToString());
                        objB.LeaveLocation = dr["Location"].ToString();
                        objB.Reason = dr["Reason"].ToString();
                        objB.Document = dr["UploadDocuments"].ToString();
                        objB.StatusId = Convert.ToInt32(dr["StatusId"].ToString());
                    ...
                    return PartialView("UpdateLeaveRequestAsyncAM", objB);
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<ActionResult> UpdateLeaveRequestAsyncAM(Bind obj)
        {
            try
            {
                if (Session["StaffCode"] == null)
                ...
                {
                    //obj.LeaveAppliedId = id; // Uncomment if id is passed as a parameter
                    var id = obj.LeaveAppliedId;
                    await BALObj.UpdateLeaveStatusAM(obj);
                    return RedirectToAction("GetLeaveRequestsAsyncAM");
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<ActionResult> ApplyCompOffAsyncAM(Bind obj)
        {
            try
            {
                if (Session["StaffCode"] == null)
                ...
                else
                {
                    obj.StaffCode = Session["StaffCode"].ToString();
                    //obj.LeaveAppliedId = id; // Uncomment if id is passed as a parameter
                    await BALObj.ApplyCompOffAM(obj);
                    return Json(new { success = true, message = "Leave Applied successfully" });
                }
            }
            catch (Exception ex)
            {
                ...
                // Return an error message
                return Json(new { success = false, message = "An error occurred while applying Comp Off." });
            }
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ActionResult> EditLeaveAsyncAM(Bind obj)
        {
            try
            {
                if (Session["StaffCode"] == null)
                ...
                else
                {
                    ...
                    await BALObj.EditLeaveDetailsAM(obj);
                    return Json(new { success = true, message = "Leave Edited successfully" });
                }
            }
            catch (Exception ex)
            {
                ...
                // Return an error message
                return Json(new { success = false, message = "An error occurred while editing leave details." });
            }
        }
